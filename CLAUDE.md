...
本リポジトリは、複数の機能を持つアプリです

初期ページはMainページです

Performanceページではコンピュータのメモリ使用量などをリアルタイムで確認します

言語はElectron、React、TypeScript、Node.jsです

起動時はスプラッシュ画面にてローディングしてください
スプラッシュは丸い形とし、弧を描くように100％になるようにしてください

ブラウザの枠は消して、アプリ独自の最小化・拡大・閉じるボタンを右上につけてください
...
UIデザインは近未来SF的なイメージで、
背景には、歯車のような円型の模様が回転しています。

改修するたびに、npm run dev:browserを行い、私が確認できるようにしてください。
...
```
filename: .playwright-screenshots/[任意のファイル名].png
```

**例：**
```
.playwright-screenshots/logo-animation-test.png
.playwright-screenshots/performance-page-20250208.png
```

**禁止事項：**
- プロジェクトルート直下へのスクリーンショット出力
- 他のディレクトリへの出力
